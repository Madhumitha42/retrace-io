...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const lostDt = document.getElementById("lost-datetime");
  const foundDt = document.getElementById("found-datetime");
  if (lostDt) lostDt.value = nowStr;
  if (foundDt) foundDt.value = nowStr;
});
...
function initMaps() {
  ...
  const lostElem = document.getElementById('map-lost');
  if (lostElem) {
  ...
  const foundElem = document.getElementById('map-found');
  if (foundElem) {
...
function updateLostCoords(lat, lng) {
  const latIn = document.getElementById("lost-lat");
  const lngIn = document.getElementById("lost-lng");
  if (latIn) latIn.value = lat;
  if (lngIn) lngIn.value = lng;
  const latDisp = document.getElementById("lost-lat-disp");
  const lngDisp = document.getElementById("lost-lng-disp");
  if (latDisp) latDisp.innerText = lat.toFixed(4);
  if (lngDisp) lngDisp.innerText = lng.toFixed(4);
}

function updateFoundCoords(lat, lng) {
  const latIn = document.getElementById("found-lat");
  const lngIn = document.getElementById("found-lng");
  if (latIn) latIn.value = lat;
  if (lngIn) lngIn.value = lng;
  const latDisp = document.getElementById("found-lat-disp");
  const lngDisp = document.getElementById("found-lng-disp");
  if (latDisp) latDisp.innerText = lat.toFixed(4);
  if (lngDisp) lngDisp.innerText = lng.toFixed(4);
}
...
async function fetchFeeds() {
  ...
  if (statLost) statLost.innerText = allLostItems.length;
  if (statFound) statFound.innerText = allFoundItems.length;
