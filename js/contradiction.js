...
async function runContradictionAnalysis() {
  ...
  try {
    ...
    renderContradictionResult(data.result);
  } catch (err) {
    // Client-side fallback contradiction analyzer for GitHub Pages static mode
    const fallbackResult = analyzeContradictionsJS(documents);
    renderContradictionResult(fallbackResult);
  }
}

function analyzeContradictionsJS(documents) {
  const dateRegex = /\b(?:\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\b/gi;
  const docDates = {};

  documents.forEach(doc => {
    const matches = doc.text.match(dateRegex);
    if (matches && matches.length) {
      docDates[doc.title] = matches;
    }
  });

  const contradictions = [];
  if (Object.keys(docDates).length >= 2) {
    const uniqueVals = new Set();
    Object.values(docDates).forEach(list => list.forEach(v => uniqueVals.add(v.toLowerCase())));

    if (uniqueVals.size > 1) {
      const details = Object.entries(docDates).map(([t, list]) => ({
        document: t,
        stated_value: list.join(", ")
      }));
      contradictions.push({
        topic: "Project Deadline / Important Dates",
        severity: "HIGH",
        confidence: "94%",
        explanation: "Conflicting date specifications detected across input documents.",
        details: details
      });
    }
  }
...
function renderContradictionResult(result) {
  const resContainer = document.getElementById("contradiction-results");
